...
def build_connection_indicator(parent: tk.Widget, palette) -> ConnectionIndicator:
    """A colored dot (red/yellow/green) plus a status label, e.g. "Connected (name@pacific.edu)".

    `palette` is a `theme.Palette`, but left untyped here -- `widgets.py` is
    deliberately kept free of any import of `theme` (or `runner`, `outlook`,
    `config`), same boundary as the rest of this module.
    """
    frame = ttk.Frame(parent)
    canvas = tk.Canvas(frame, width=14, height=14, highlightthickness=0, bg=palette.bg)
    dot_id = canvas.create_oval(2, 2, 12, 12, fill=palette.status_checking, outline="")
    canvas.pack(side="left", padx=(0, 6))
    status_var = tk.StringVar(value="Checking Outlook...")
    ttk.Label(frame, textvariable=status_var).pack(side="left")
    return ConnectionIndicator(
        frame=frame, status_var=status_var, canvas=canvas, dot_id=dot_id
    )


def set_connection_status(
    indicator: ConnectionIndicator, palette, state: str, text: str
) -> None:
    """Update the dot's color (`state` is "checking"/"connected"/"disconnected") and its label."""
    colors = {
        "checking": palette.status_checking,
        "connected": palette.status_connected,
        "disconnected": palette.status_disconnected,
    }
    indicator.canvas.itemconfig(indicator.dot_id, fill=colors[state])
...
def build_report_checkboxes(
    parent: tk.Widget, reports: list[tuple[str, str]]
) -> ReportCheckboxes:
    ...
    return ReportCheckboxes(
        frame=frame, report_vars=report_vars, select_all_var=select_all_var
    )
...
# to auto-detecting the pay period from today's date and the first-Sunday
# setting, same as the CLI's default (no `--pay-period` passed).
...
def build_directory_picker(
    parent: tk.Widget, label_text: str, initial_value: str
) -> DirectoryPicker:
...
def build_file_picker(
    parent: tk.Widget, label_text: str, initial_value: str
) -> DirectoryPicker:
    """Like `build_directory_picker`, but Browse... picks a single file."""
    frame = ttk.Frame(parent)
    ttk.Label(frame, text=label_text, width=14).pack(side="left")
    path_var = tk.StringVar(value=initial_value)
    entry = ttk.Entry(frame, textvariable=path_var, state="readonly")
...
def build_text_field(
    ...
    """A label and an editable text entry, aligned with the picker rows.

    `width=48` sets the entry's *requested* size so the Settings dialog
    opens wide enough for URLs/date lists; `fill="x"` still lets it grow
    with the dialog.
    """
    frame = ttk.Frame(parent)
    ttk.Label(frame, text=label_text, width=14).pack(side="left")
    value_var = tk.StringVar(value=initial_value)
    ttk.Entry(frame, textvariable=value_var, width=48).pack(
        side="left", fill="x", expand=True, padx=(0, 4)
    )
    return TextField(frame=frame, value_var=value_var)
...
def build_run_controls(
    ...
    progress_bar = ttk.Progressbar(
        frame, mode="determinate", maximum=1, value=0, length=150
    )
...
# by `_apply_log_palette` since sv_ttk can't reach a plain `tk.Text`'s colors.
_WARNING_LEVELS = {"WARNING"}
_ERROR_LEVELS = {"ERROR", "CRITICAL"}


def build_log_pane(parent: tk.Widget, palette) -> LogPane:
    ...
    log_pane = LogPane(frame=frame, text=text)
    _apply_log_palette(log_pane, palette)
    return log_pane


def _apply_log_palette(log_pane: LogPane, palette) -> None:
    """Push `palette` into the log pane's colors, including the states
    sv_ttk can't reach on a plain `tk.Text`: selection highlight (otherwise
    falls back to the OS default) and a visible keyboard-focus ring
    (otherwise `highlightthickness=0` would leave tab-focus invisible).
    """
    log_pane.text.configure(
        ...
        highlightbackground=palette.border,  # unfocused ring
        highlightcolor=palette.selection_bg,  # focused ring
    )
