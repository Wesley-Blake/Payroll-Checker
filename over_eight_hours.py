...
def over_eight_hours(df: DataFrame, emails_df: DataFrame) -> dict[str, list[str]] | None:
    ...
    # Get final_df
    WHITE_LIST = ["Empl_ID", "LastName", "JobECLS", "earn_code", "ts_entry_date", "appr_id", "earning_hours"]
    new_order_df = df[WHITE_LIST]
    filtered_df = new_order_df.groupby(WHITE_LIST[:-1], as_index=False)["earning_hours"].sum() #type: ignore
    pre_final_df = filtered_df[(filtered_df[WHITE_LIST[3]] == "REG") &
                           (
                           ((filtered_df[WHITE_LIST[2]] == "UU") & (filtered_df[WHITE_LIST[-1]] > 7.5)) |
                           ((filtered_df[WHITE_LIST[2]] == "VV") & (filtered_df[WHITE_LIST[-1]] > 7.5)) |
                           ((filtered_df[WHITE_LIST[2]] != "UU") & (filtered_df[WHITE_LIST[-1]] > 8)) |
                           ((filtered_df[WHITE_LIST[2]] != "VV") & (filtered_df[WHITE_LIST[-1]] > 8))
                           )
                           ]
    if pre_final_df.empty: return None

    # Get dict
    EMAIL_WHITE_LIST = ["EmplID", "PacificEmail", "SupervisorEmail"]
    result: dict[str,list[str]] = {}
    
    ordered_email_df = emails_df[EMAIL_WHITE_LIST].drop_duplicates()
    final_df = pd.merge(pre_final_df, ordered_email_df, left_on="Empl_ID", right_on="EmplID", how="inner") # type: ignore
    ...
    for manager_email in manager_emails: # type: ignore
        ...
        result[manager_email] += final_df[final_df[headers[-1]] == manager_email][headers[-2]].unique().tolist() # type: ignore
